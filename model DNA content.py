...
# function to create 4 spermatids with normal segregation
# in meiosis I and II
def spermatid_wt():
    #create spermatids
...
# function to create 4 spermatids with random segregation
# in meiosis I but normal segregation in meiosis II
...
# function to create 4 spermatids with random segregation
# of autosomes in meiosis I but normal segregation in meiosis II
...
# function that converst the volume of a sphere into the area
# of the equator disc
...
for mode in ['wt', 'random', 'autosomal_random']:
    ...
    for i in range(20):
        cyst = make_cyst(mode)
        datat = {'ObjectNumber': range(1,65), 'norm signal': cyst}
        cyst_df = pd.DataFrame(data = datat)
        # optional: mimic limits of the DNA content analyses
        # by removing all nuclei that have less than 10Mb of DNA
        # cyst_df = cyst_df.loc[cyst_df['norm signal'] >= 10]
        cyst_df["CystNumber"] = i+1
        cyst_df["cyst_stdev"] = np.std(cyst)
        mode_df = mode_df.append(cyst_df)
    mode_df["genotype"] = "model, " + mode
    #calcute mean standard deviation
    mean_cyst_stdev = mode_df.drop_duplicates(
        subset ="CystNumber", keep = "first",
        inplace = False)["cyst_stdev"].mean(axis=0)
    mode_df["mean_cyst_stdev"] = mean_cyst_stdev
...
save_path = filedialog.asksaveasfilename(
    title='Save modeled results as ...',
    defaultextension = '.xlsx',
    initialfile = "modeled DNA content results")
