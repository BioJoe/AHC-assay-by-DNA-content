...
for gt in gt_ls:
    cyst_ls = df_cells.loc[df_cells['genotype'] == gt].CystNumber.unique()
    for cyst in cyst_ls:
        cyst_stdev = df_cells.loc[(df_cells['genotype'] == gt) & (df_cells['CystNumber'] == cyst), 'norm signal'].std(axis=0)
        df_cells.loc[(df_cells['genotype'] == gt) & (df_cells['CystNumber'] == cyst), 'cyst_stdev'] = cyst_stdev

    #calcute mean standard deviation each genotype
    mean_cyst_stdev = df_cells.loc[df_cells['genotype'] == gt].drop_duplicates(subset ="CystNumber", keep = "first", inplace = False)["cyst_stdev"].mean(axis=0)
    df_cells.loc[(df_cells['genotype'] == gt), 'mean_cyst_stdev'] = mean_cyst_stdev
